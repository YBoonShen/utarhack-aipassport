...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-full lg:w-60 bg-navy-header shrink-0 flex flex-col p-4 lg:p-5">
      ...
      <nav className="mt-4 lg:mt-7 flex flex-row lg:flex-col gap-1.5 overflow-x-auto lg:overflow-x-visible">
        {nav.map(n => (
          <NavLink
            key={n.to}
            to={n.to}
      ...
      <button onClick={() => setLogoutOpen(true)} className="text-[#cbd5e1] text-[11px] px-1.5 mt-3 lg:mt-0 mb-0 lg:mb-3 text-left cursor-pointer hover:text-white">← Log out</button>
      {/* Identity card is desktop chrome — hidden on phones to keep the strip short. */}
      <div className="bg-[#173976] rounded-[12px] p-3 hidden lg:flex items-center gap-3">

...
export default function TrainingModules() {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 lg:px-10 py-6 lg:py-8">
      <h1 className="text-[26px] lg:text-[30px] font-bold text-navy-header">All Training Modules</h1>
      <p className="text-[#667085] text-sm mt-2 max-w-[900px]">
        Complete modules to earn XP, stamps and unlock AI tools. Every module ends with a type-your-own practice. Each
        module contributes its best result to your total XP — retaking one can only raise that contribution.
      </p>
      ...
      <div className="flex flex-col gap-4 mt-6">
        {MODULE_LIST.map((m, i) => {
          ...
          return (
            <Link
              ...
              className="bg-white border border-[#e0e0e5] rounded-[16px] min-h-[104px] sm:h-[104px] px-4 sm:px-6 py-4 sm:py-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:border-navy-header"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gold-brand font-medium text-[12.5px] mt-1.5">
                  ...
                  {done ? `${record?.pointsEarned ?? 0} / ${m.points} XP earned${record?.attempts > 1 ? ` · ${record.attempts} attempts` : ''}` : `+${m.points} XP`}
                </p>
              </div>
              {done ? (
        ...
        <div className="bg-[#f5f5f7] border border-[#e0e0e5] rounded-[16px] min-h-[104px] sm:h-[104px] px-4 sm:px-6 py-4 sm:py-0 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <span className="bg-[#e5e5eb] text-[#667085] font-semibold text-sm h-11 px-5 rounded-full inline-flex items-center shrink-0 ml-auto sm:ml-0">Locked</span>
        </div>
      </div>
    </div>
  )
